...
public class EntityBee extends EntityAnimal implements IEntityAngerable, EntityBird {
    ...
    public boolean doHurtTarget(Entity entity) {
        boolean flag = entity.hurt(this.damageSources().sting(this), (float) ((int) this.getAttributeValue(GenericAttributes.ATTACK_DAMAGE)));
    ...
    private class k extends EntityBee.a {
        ...
        private final Predicate<IBlockData> VALID_POLLINATION_BLOCKS = (iblockdata) -> {
            return iblockdata.hasProperty(BlockProperties.WATERLOGGED) && (Boolean) iblockdata.getValue(BlockProperties.WATERLOGGED) ? false : (iblockdata.is(TagsBlock.FLOWERS) ? (iblockdata.is(Blocks.SUNFLOWER) ? iblockdata.getValue(BlockTallPlant.HALF) == BlockPropertyDoubleBlockHalf.UPPER : true) : false);
        };
                    ...
                    if (iblockdata.is(TagsBlock.BEE_GROWABLES)) {
                        if (block instanceof BlockCrops) {
                            ...
                            if (block instanceof BlockStem) {
                                ...
                                if (j < 3) {
                                    iblockdata1 = (IBlockData) iblockdata.setValue(BlockSweetBerryBush.AGE, j + 1);
                                }
