...
public class PlayerChunkMap extends IChunkLoader implements PlayerChunk.d {
    ...
    private static boolean isChunkOnRangeBorder(int i, int j, int k, int l, int i1) {
        return !isChunkInRange(i, j, k, l, i1) ? false : !isChunkInRange(i + 1, j + 1, k, l, i1) || !isChunkInRange(i - 1, j + 1, k, l, i1) || !isChunkInRange(i + 1, j - 1, k, l, i1) || !isChunkInRange(i - 1, j - 1, k, l, i1);
    }
    ...
    public CompletableFuture<Either<Chunk, PlayerChunk.Failure>> prepareTickingChunk(PlayerChunk playerchunk) {
        ...
        completablefuture1.thenAcceptAsync((either) -> {
            either.ifLeft((chunk) -> {
                ...
                this.getPlayers(playerchunk.getPos(), false).forEach((entityplayer) -> {
        ...
        if (Math.abs(i1 - i) <= k1 * 2 && Math.abs(j1 - j) <= k1 * 2) {
            ...
            for (int l2 = l1; l2 <= j2; ++l2) {
                for (int i3 = i2; i3 <= k2; ++i3) {
                    ...
                    this.updateChunkTracking(entityplayer, new ChunkCoordIntPair(l2, i3), new MutableObject(), flag3, flag4);
                }
            }
        } else {
            boolean flag5;
            ...
            for (l1 = i1 - k1; l1 <= i1 + k1; ++l1) {
                for (i2 = j1 - k1; i2 <= j1 + k1; ++i2) {
                    if (isChunkInRange(l1, i2, i1, j1, this.viewDistance)) {
                        flag5 = true;
                        flag6 = false;
                        this.updateChunkTracking(entityplayer, new ChunkCoordIntPair(l1, i2), new MutableObject(), true, false);
                    }
                }
            }
            ...
            for (l1 = i - k1; l1 <= i + k1; ++l1) {
                for (i2 = j - k1; i2 <= j + k1; ++i2) {
                    if (isChunkInRange(l1, i2, i, j, this.viewDistance)) {
                        flag5 = false;
                        flag6 = true;
                        this.updateChunkTracking(entityplayer, new ChunkCoordIntPair(l1, i2), new MutableObject(), false, true);
                    }
                }
            }
        }
    ...
    private void playerLoadedChunk(EntityPlayer entityplayer, MutableObject<ClientboundLevelChunkWithLightPacket> mutableobject, Chunk chunk) {
        if (mutableobject.getValue() == null) {
            mutableobject.setValue(new ClientboundLevelChunkWithLightPacket(chunk, this.lightEngine, (BitSet) null, (BitSet) null));
        }
